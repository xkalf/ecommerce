...
import { and, count, eq, ilike, lte, sql } from "drizzle-orm/sql";
...
export async function getCategoryProducts() {
	const rankedProducts = db.$with("ranked_products").as(
		db
			.select({
				...
				productRank:
					sql`ROW_NUMBER() OVER(PARTITION BY ${products.category} ORDER BY ${products.createdAt} DESC)`.as(
						"product_rank",
					),
			})
			.from(products),
	...
	const data = await db
		...
		.select()
		.from(rankedProducts)
		.where(lte(rankedProducts.productRank, 5))
		.orderBy(rankedProducts.category, desc(rankedProducts.createdAt));
