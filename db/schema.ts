...
// COMMONS
export const commonTypeEnum = pgEnum("common_type", [
	"category",
	"brand",
	"banner",
]);

export const commons = pgTable("commons", {
	id: uuid("id").defaultRandom().primaryKey().notNull(),
	name: text("name"),
	image: text("images"),
	type: commonTypeEnum("type").notNull(),
	parentId: uuid("parentId").references((): AnyPgColumn => commons.id, {
		onDelete: "cascade",
	}),
});
